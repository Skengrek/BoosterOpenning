...
export const API = defineStore('API', {
    ...
    actions: {
        ...
        async register(username, password, email) {
            const response = await this.callAPI(
                'post',
                'http://backend:8001/api/users/create/',
                { "content-type": "application/json" },
                {
                    username: username,
                    password: password,
        ...
        async refreshToken() {
            const resp = await this.callAPI(
                'POST',
                'http://backend:8001/api/token/refresh/',
                { "content-type": "application/json" },
        ...
        async getCardExample(number) {
            const response = await this.callAPI(
                'GET',
                `http://backend:8001/api/cards/example/${number}`,
                {
                    "content-type": "application/json",
        ...
        async listBoosters() {
            ...
            const response = await this.callAPI(
                'GET',
                'http://backend:8001/api/cards/booster/user/list/boosters',
                {
                    "content-type": "application/json",
        ...
        async listCards() {
            ...
            const response = await this.callAPI(
                'GET',
                'http://backend:8001/api/cards/booster/user/list/cards',
                {
                    "content-type": "application/json",
                    "Authorization": `Bearer ${this.access}`,
                    'Access-Control-Allow-Origin': "*",
        ...
        async openBooster(extension_id) {
            ...
            const response = await this.callAPI(
                'GET',
                `http://backend:8001/api/cards/booster/user/open/${extension_id}`,
                {
                    "content-type": "application/json",
                    "Authorization": `Bearer ${this.access}`,
                    'Access-Control-Allow-Origin': "*",
